...
import { MOCK_DOCUMENTS, MOCK_VIDEOS, MOCK_FORUM_POSTS, MOCK_USERS } from '../constants';
...
interface DataContextType {
  ...
  allUsers: any[]; // User database for admin
  ...
  updateUserStatus: (id: string, updates: any) => void;
}
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const storedUsers = localStorage.getItem('smartstudy_all_users');
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem('smartstudy_all_users', JSON.stringify(allUsers));
    }
  }, [allUsers, isLoaded]);
  ...
  // User Actions (Admin)
  const updateUserStatus = (id: string, updates: any) => {
