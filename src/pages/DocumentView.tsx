...
import {
  Download, MessageSquare, ChevronLeft, Lock, FileText, Bot,
  HelpCircle, Bookmark, LogIn, UserPlus, Sparkles, Eye,
  Maximize, Minimize, CheckCircle, Loader2,
...
const DocumentView: React.FC = () => {
  ...
  const [chatHistory, setChatHistory] = useState<{ role: string, text: string, partial?: boolean, notice?: string }[]>([]);
  ...
  useEffect(() => {
    ...
    aiTutorAPI.chat(
      ...
      .then(res => {
        ...
        // A metadata overview (partial) renders WITH its notice instead.
        if (res.partial) {
          setSummary(res.response);
          setSummaryNotice(res.notice || 'General overview — not read from the file.');
        } else if (res.grounded === false) {
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, {
        ...
        ...(response.partial ? { partial: true as const, notice: response.notice } : {}),
      }]);
    } catch (error: any) {
                ...
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-3.5 mb-4">
                  ...
                  {isSummaryLoading ? (
                    ...
                    <div className="py-1">
                      ...
                      <button
                        onClick={() => { setSummaryUnavailable(null); setSummaryNotice(null); setSummaryAttempt(a => a + 1); }}
                    ...
                    <div className="text-[13px] text-inksoft leading-relaxed">
                      {summaryNotice && (
                        <p className="mb-2 px-2.5 py-1.5 rounded-lg bg-amber-50 border border-amber-200 text-[11px] font-medium text-amber-800 leading-relaxed">
                          {summaryNotice}
                        </p>
                      )}
                    ...
                    msg.role === 'user' ? (
                      ...
                      <div key={i} className="flex gap-2.5 animate-fade-in">
                        ...
                        <div className="max-w-[88%] px-3.5 py-2.5 rounded-2xl rounded-tl-md bg-surface text-ink border border-zinc-200 text-sm leading-relaxed shadow-sm">
                          <MarkdownRenderer content={msg.text} />
                          {msg.partial && msg.notice && (
                            <p className="mt-2 pt-2 border-t border-zinc-100 text-[11px] text-zinc-500 leading-relaxed">
                              {msg.notice}
                            </p>
                          )}
                        </div>
          ...
          {activeTab === 'chat' && (
            <div className="p-3 bg-surface border-t border-zinc-100 flex-shrink-0">
              <ChatInput
                ...
                placeholder="Ask a follow-up, or paste an image…"
                disabled={isChatLoading}
                onProcessingChange={setImageBusy}
                imageInputId="document-image-upload-input"
