    ...
    <div className="flex flex-col min-h-screen">
      <div className="flex-1 animate-fade-in max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="text-center mb-12 sm:mb-16">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-ink mb-4 sm:mb-6">Join our mission</h1>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mb-12 sm:mb-16 md:mb-20">
           <div className="bg-zinc-50 p-4 sm:p-6 md:p-8 rounded-2xl border border-zinc-100">
              <h3 className="font-bold text-ink mb-3 text-base sm:text-lg">Impact at Scale</h3>
              <p className="text-xs sm:text-sm text-inksoft leading-relaxed">Your work will directly help students across Ethiopia succeed in their national exams.</p>
           </div>
           <div className="bg-zinc-50 p-4 sm:p-6 md:p-8 rounded-2xl border border-zinc-100">
              <h3 className="font-bold text-ink mb-3 text-base sm:text-lg">Remote-First Culture</h3>
              <p className="text-xs sm:text-sm text-inksoft leading-relaxed">We value output over hours. Work from anywhere in Ethiopia (or the world) with flexible schedules.</p>
           </div>
           <div className="bg-zinc-50 p-4 sm:p-6 md:p-8 rounded-2xl border border-zinc-100">
              <h3 className="font-bold text-ink mb-3 text-base sm:text-lg">Growth & Learning</h3>
              <p className="text-xs sm:text-sm text-inksoft leading-relaxed">We invest in our team. Get access to courses, books, and mentorship to grow your career.</p>
           </div>
        </div>

        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-ink mb-6 sm:mb-8 flex items-center gap-2">
            <Briefcase className="text-inksoft w-5 h-5 sm:w-6 sm:h-6" /> Open Positions
          </h2>

          {loading ? (
            <div className="space-y-3 sm:space-y-4">
              {[1, 2, 3].map(i => (
                <div key={i} className="bg-surface border border-zinc-200 rounded-xl p-4 sm:p-6 animate-pulse">
            ...
            <div className="space-y-3 sm:space-y-4">
              {positions.map((position) => {
                ...
                return (
                  <div key={position.id} className="bg-surface border border-zinc-200 rounded-xl p-4 sm:p-6 hover:border-zinc-300 hover:shadow-md transition-all group">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
                      <div className="flex gap-3 sm:gap-4">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-zinc-100 rounded-lg flex items-center justify-center text-zinc-500 group-hover:bg-zinc-900 group-hover:text-onink transition-colors flex-shrink-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          >
                            <h3 className="font-bold text-base sm:text-lg text-ink group-hover:text-ink transition-colors hover:text-inksoft">{position.title}</h3>
                          </button>
                          ...
                          <p className="text-xs sm:text-sm text-inksoft mt-2 line-clamp-2">{position.description}</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleViewDetails(position)}
                          className="px-3 sm:px-4 py-2 bg-zinc-100 text-inksoft rounded-lg font-medium opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100 focus-visible:opacity-100 transition-all flex items-center gap-1 sm:gap-2 text-xs sm:text-sm hover:bg-zinc-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900/30"
                        >
                        ...
                        <button
                          ...
                          className="px-4 sm:px-6 py-2 bg-zinc-900 text-onink rounded-lg font-medium opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100 focus-visible:opacity-100 transition-all flex items-center gap-1 sm:gap-2 text-sm sm:text-base w-fit focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900/30"
                        >
          ...
          <div className="mt-8 sm:mt-12 text-center bg-zinc-50 rounded-xl p-6 sm:p-8 border border-zinc-200">
             <p className="text-inksoft mb-4 text-sm sm:text-base">Don't see a role that fits? We're always looking for talent.</p>
             <a href="mailto:careers@smartstudy.com" className="text-ink font-bold hover:underline inline-flex items-center gap-2 text-sm sm:text-base">
            ...
            <div className="p-4 sm:p-6 overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-xl text-ink">{detailPosition.title}</h3>
                ...
                <button
                  ...
                  className="p-2 text-zinc-400 hover:text-inksoft rounded-lg hover:bg-zinc-100 transition-colors"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                {/* Description */}
                <div>
                  <h4 className="font-semibold text-ink mb-2 text-sm">About the Role</h4>
                  <p className="text-sm text-inksoft leading-relaxed whitespace-pre-wrap">{detailPosition.description}</p>
                </div>

                {/* Requirements */}
                {detailPosition.requirements && (
                  <div>
                    <h4 className="font-semibold text-ink mb-2 text-sm">Requirements & Qualifications</h4>
                    <div className="bg-zinc-50 rounded-lg p-4 border border-zinc-100">
                      <p className="text-sm text-inksoft leading-relaxed whitespace-pre-wrap">{detailPosition.requirements}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-3 pt-4 border-t border-zinc-100">
                  <button
                    ...
                    className="flex-1 px-4 py-2 bg-surface border border-zinc-200 text-inksoft font-medium rounded-lg hover:bg-zinc-50 transition-colors"
                  >
                    Close
                  </button>
                  <button
                    onClick={handleApplyFromDetail}
                    className="flex-1 px-4 py-2 bg-zinc-900 text-onink font-medium rounded-lg hover:bg-zinc-800 transition-colors flex items-center justify-center gap-2"
                  >
                    <CheckCircle size={16} />
                    Apply Now
                  </button>
        ...
        {selectedPosition ? (
            <div className="p-4 sm:p-6 overflow-y-auto">
              ...
              <form onSubmit={handleApplicationSubmit} className="space-y-4">
                <div>
                  <label htmlFor="apply-name" className="block text-sm font-medium text-inksoft mb-1">Full Name *</label>
                  <input
                    id="apply-name"
                    type="text"
                    autoComplete="name"
                ...
                <div>
                  <label htmlFor="apply-email" className="block text-sm font-medium text-inksoft mb-1">Email *</label>
                  <input
                    id="apply-email"
                    type="email"
                    autoComplete="email"
                ...
                <div>
                  <label htmlFor="apply-phone" className="block text-sm font-medium text-inksoft mb-1">Phone Number</label>
                  <input
                    id="apply-phone"
                    type="tel"
                    autoComplete="tel"
                ...
                <div>
                  <label htmlFor="apply-resume" className="block text-sm font-medium text-inksoft mb-1">Resume URL</label>
                  <input
                    id="apply-resume"
                    type="url"
                    autoComplete="url"
                ...
                <div>
                  <label htmlFor="apply-cover" className="block text-sm font-medium text-inksoft mb-1">Cover Letter</label>
                  <textarea
                    id="apply-cover"
