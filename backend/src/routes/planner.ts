...
// Batch helpers live in services/plannerBatch (pure + unit-tested); this
// route re-exports them so existing importers keep working.
export {
  normalizeSubject,
  validateBatchEvent,
  validateBatchEvents,
...
// Batch-create study events (one round trip instead of N sequential POSTs).
// Thin by design: validation + SQL building live in services/plannerBatch;
// this handler only wires HTTP ↔ DB and never leaks pg internals.
